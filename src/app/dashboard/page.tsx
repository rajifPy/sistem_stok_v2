// src/app/dashboard/page.tsx - Enhanced Responsive
'use client';

import { useEffect, useState } from 'react';
import Layout from '@/components/Layout';
import { formatCurrency } from '@/lib/utils';
import { Package, TrendingUp, ShoppingCart, DollarSign, ArrowUp, ArrowDown, RefreshCw } from 'lucide-react';
    ...
    <Layout>
      <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto min-h-screen bg-gray-50">
        ...
        <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <button
            onClick={fetchData}
            className="w-full sm:w-auto px-4 py-2.5 bg-white border border-gray-200 rounded-xl hover:shadow-md transition-all flex items-center justify-center gap-2 text-gray-700 font-semibold"
          >
            <RefreshCw size={18} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
          {cards.map((card, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer"
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  {card.change !== undefined && (
                    <div className={`flex items-center gap-1 text-xs sm:text-sm font-semibold ${
                      ...
                      {card.change >= 0 ? <ArrowUp size={14} /> : <ArrowDown size={14} />}
                      {Math.abs(card.change).toFixed(1)}%
                    </div>
                  )}
                </div>
                <p className="text-xs sm:text-sm text-gray-600 mb-1">{card.title}</p>
                <p className="text-xl sm:text-3xl font-bold text-gray-900 mb-1 truncate">{card.value}</p>
                {card.subValue && (
                  <p className="text-xs sm:text-sm text-gray-500 truncate">{card.subValue}</p>
                )}
              </div>
            );
          })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-6">
          {/* Revenue Chart */}
          <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 p-4 sm:p-6 hover:shadow-lg transition-shadow">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
              <div>
                <h3 className="text-base sm:text-lg font-bold text-gray-900">Pendapatan & Profit</h3>
                <p className="text-xs sm:text-sm text-gray-600">Trend {selectedPeriod === 'week' ? '7 hari' : '30 hari'}</p>
              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 hover:shadow-lg transition-shadow">
            <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-2">Kategori</h3>
            <p className="text-xs sm:text-sm text-gray-600 mb-4">Distribusi revenue</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 hover:shadow-lg transition-shadow">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-base sm:text-lg font-bold text-gray-900">⚠️ Stok Rendah</h3>
                <p className="text-xs sm:text-sm text-gray-600">Produk perlu restock</p>
              </div>
              <span className="px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs sm:text-sm font-bold">
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto">
              {lowStockProducts.length === 0 ? (
                <div className="text-center py-8 text-gray-400">
                  ...
                  <p className="font-semibold text-sm">Semua stok aman! 🎉</p>
                </div>
              ) : (
                lowStockProducts.map((product, index) => (
                  <div
                    key={product.id}
                    className="flex items-center justify-between p-3 sm:p-4 bg-gradient-to-r from-red-50 to-orange-50 rounded-xl hover:shadow-md transition-all"
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-red-100 rounded-full flex items-center justify-center text-red-600 font-bold text-sm sm:text-base flex-shrink-0">
                        {index + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-900 text-sm sm:text-base truncate">{product.nama_produk}</p>
                        <p className="text-xs sm:text-sm text-gray-600">{product.kategori}</p>
                      </div>
                    </div>
                    <div className="text-right ml-2">
                      <p className="text-xl sm:text-2xl font-bold text-red-600">{product.stok}</p>
          ...
          {/* Revenue Summary */}
          <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-xl p-4 sm:p-6 text-white hover:shadow-lg transition-shadow">
            <h3 className="text-base sm:text-lg font-bold mb-4 sm:mb-6">💰 Ringkasan Finansial</h3>
            
            <div className="space-y-3 sm:space-y-4">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4">
                <span className="text-xs sm:text-sm text-blue-100">Pendapatan Hari Ini</span>
                <p className="text-xl sm:text-3xl font-bold mt-1">{formatCurrency(stats.todayRevenue)}</p>
              </div>

              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4">
                <span className="text-xs sm:text-sm text-blue-100">Profit Hari Ini</span>
                <p className="text-xl sm:text-3xl font-bold text-green-300 mt-1">{formatCurrency(stats.totalProfit)}</p>
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-2 sm:p-3">
                  ...
                  <p className="text-sm sm:text-lg font-bold truncate">{formatCurrency(stats.yesterdayRevenue)}</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-2 sm:p-3">
                  <p className="text-xs text-blue-200 mb-1">Perubahan</p>
                  <p className={`text-sm sm:text-lg font-bold ${revenueChange >= 0 ? 'text-green-300' : 'text-red-300'}`}>
